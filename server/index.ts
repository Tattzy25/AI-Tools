...
app.post('/api/analyze-image', upload.single('image'), async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (isImage) {
      ...
      try {
        const resp = await groq.chat.completions.create({ model: 'meta-llama/llama-4-scout-17b-16e-instruct', messages: summaryMsg, temperature: 0.3 })
...
app.post('/api/generate-data', async (req: express.Request, res: express.Response) => {
  try {
