...
const app = express()
app.use(cors({ origin: ['http://localhost:5173', 'https://bridgit-ai.com'], credentials: false }))
...
app.post('/api/analyze-image', upload.single('image'), async (req: express.Request, res: express.Response) => {
  try {
    ...
    const messages: any[] = [
      {
        role: 'system',
        content:
          'Return STRICT JSON describing the image ONLY. Include: image_description, objects_detected[{object,confidence}], color_palette[{color,hex,percentage}], text[], confidence. Do NOT include provider/model/processing metadata. JSON only, no code fences.',
      },
      {
        role: 'user',
        content: [
          { type: 'text', text: 'Analyze this image and return the JSON described.' },
    ...
    const colors = Array.isArray(aiJson?.color_palette)
      ? aiJson.color_palette.map((c: any) => ({ name: String(c.color||''), percentage: Number(c.percentage||0), hex: String(c.hex||'') }))
      : []
    const objects = Array.isArray(aiJson?.objects_detected)
      ? aiJson.objects_detected.map((o: any) => ({ name: String(o.object||''), confidence: Number(o.confidence||0) }))
      : []
    ...
    if (typeof aiJson?.confidence === 'number') {
      response.visual_analysis.ai_analysis.confidence = Number(aiJson.confidence)
    }
