...
app.post('/api/analyze-image', upload.single('image'), async (req, res) => {
  try {
    ...
    const messages = [
      { role: 'system', content: 'Return STRICT JSON describing the image ONLY. Include: image_description, objects_detected[{object,confidence}], color_palette[{color,hex,percentage}], text[], confidence. Do NOT include provider/model/processing metadata. JSON only.' },
      { role: 'user', content: [ { type: 'text', text: 'Analyze this image and return the JSON described.' }, { type: 'image_url', image_url: { url: dataUrl } } ] },
    ]
